...
Se sei un utente master o il proprietario di un account, puoi abilitare il tracciamento della password e impostare una password per l'accesso singolo all'account. Con il tracciamento della gli utenti possono archiviare i dati della password del software per i dispositivi e il loro software associato.
{:shortdesc}

## Abilitazione del tracciamento delle password
{: #customerportal_enabpwtrak}

Il portale clienti contiene uno strumento di tracciamento delle password facoltativo per ogni account. Puoi utilizzare lo strumento per richiamare il tuo nome utente e la tua password, se perdi o dimentichi tali informazioni.

I team di supporto utilizzano il tracciamento delle password anche se è richiesto l'accesso remoto a un sistema. I nomi utente e le password vengono utilizzati dal Supporto solo quando necessario.

Il tracciamento delle password all'interno del portale clienti è facoltativo. Qualsiasi utente con le autorizzazioni appropriate può visualizzare tutte le password memorizzate da questo strumento. Le informazioni su utente e password vengono tracciate manualmente e pertanto non vengono sincronizzate automaticamente con un dispositivo o il suo software. Pertanto, assicurati di aggiornare lo strumento di tracciamento delle password quando aggiorni utenti e password su dispositivi e software. Utilizza la seguente procedura per aggiungere un utente allo strumento di tracciamento delle password.

1. Accedi al portale clienti utilizzando le tue credenziali univoche.
2. Fai clic su **Devices** > **Manage** > **Passwords** dal menu.
3. Fai clic sulla scheda **Add Credential**.
4. Seleziona il nome dispositivo (**Device Name**) a cui è associato l'utente dall'elenco **Device Name**.
...
Dopo aver aggiunto un utente, puoi modificare i dettagli associati all'utente o alla password. Utilizza la seguente procedura per modificare le informazioni per un utente nello strumento di tracciamento delle password.
...
5. Aggiorna i campi **user name** o **Password** come necessario.
...
2. Immetti il tuo nome utente attuale.
3. Controlla la tua email per trovare un link per reimpostare la password. Questo link è valido per 24 ore.
4. Seleziona e rispondi alle tre domande di sicurezza.

Hai cinque tentativi per rispondere alle domande di sicurezza. Se effettui più di cinque tentativi, il modulo di reimpostazione della password viene bloccato per 15 minuti prima che tu possa riprovare.
