...
Se você usar o IBMid para autenticação, quando fizer seu primeiro pedido ou for incluído como um usuário em uma conta, receberá um e-mail contendo um link para iniciar com o IBMid. Se você perder ou esquecer seu nome de usuário ou senha, acesse seu [perfil do IBMid ![Ícone de link externo](../icons/launch-glyph.svg)](https://www.ibm.com/account/profile){:new_window} e reconfigure ou recupere a senha usando as instruções que seguem o processo de conexão. Você será solicitado a inserir informações específicas, o que pode incluir o fornecimento de respostas para suas perguntas de segurança.

Se não usar o IBMid para autenticação, quando fizer seu primeiro pedido ou for incluído como um usuário em uma conta do [portal do cliente ![Ícone de link externo](../icons/launch-glyph.svg)](https://control.softlayer.com/){:new_window}, você receberá um e-mail contendo seu nome de usuário e senha inicial para iniciar no portal do cliente. Certifique-se de mudar sua senha após efetuar login pela primeira vez editando seu perfil do usuário. 

Se você esquecer sua senha depois de efetuar login, use o recurso **Senha esquecida ** que está disponível na tela de login do portal do cliente. Você receberá uma solicitação para inserir informações específicas, incluindo um conjunto de perguntas de segurança especificadas ao configurar o perfil do usuário.
...
Ao efetuar login com seu IBMid, o erro "Não reconhecemos esse IBMid ou e-mail" é exibido. Certifique-se de inserir um endereço de e-mail completo. Além disso, certifique-se de que não esteja usando seu nome de usuário de infraestrutura do {{site.data.keyword.BluSoftlayer_notm}}.
...
Embora você seja responsável por manter a consistência de dados entre servidores reais, o {{site.data.keyword.BluSoftlayer_full}} fornece uma rede privada que pode ser usada para sincronização sem incorrer em encargos de uso.
