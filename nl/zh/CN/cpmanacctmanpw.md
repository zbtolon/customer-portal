...
在客户门户网站中跟踪密码是可选的，并且任何具有相应许可权的用户都可以查看使用此工具存储的所有密码。用户和密码信息是手动跟踪的，所以不会自动与设备或其软件同步。因此，请务必在设备和软件上更新用户和密码时，同时更新密码跟踪工具。使用以下步骤向密码跟踪工具添加用户。
...
要通过密码跟踪工具查看、编辑或删除用户信息，可以在列表跨多行或多页时，使用过滤来快速找到用户。使用以下步骤在密码跟踪工具中按设备、软件或用户进行过滤。
...
要配置帐户，必须先设置 Verisign 的“VIP Access”应用程序。如果尚未执行此操作，请首先为以下某个设备下载该应用程序：
...
然后，完成以下步骤：
1. 打开应用程序。在顶部找到“凭证标识”，并在底部找到当前的一次性密码。初始可以忽略该密码，但您需要门户网站的“凭证标识”才能使应用程序保持打开状态。
