...
환경을 모니터링하는 것은 언제든지 디바이스를 검사할 수 있는 기능이 있으며 디바이스 중 하나가 작동이 중지되면 자동으로 알림이 표시됨을 의미합니다. 또한 시스템이 계속해서 원활하게 실행되도록 시스템 이벤트를 모니터링할 수 있습니다.   
{: shortdesc}
...
같은 다수의 채널을 유지보수하여 모든 스케줄링된 유지보수 이벤트 및 비상 유지보수 이벤트에 대한 알림을 계속 제공합니다. 또한 이벤트 관리 시스템에서 [이메일 알림 구독](/docs/customer-portal/cpsub2not.html){:new_window}을 수행할 수 있습니다. 이 무료 서비스는 서비스에 영향을 줄 수 있는 계획되지 않은 이벤트에 대해 구독한 사용자에게 자동으로 이메일을 발송합니다. 

### {{site.data.keyword.BluSoftlayer_notm}} 인프라 모바일 사용
{: #cp_bmxinframobile}

[{{site.data.keyword.BluSoftlayer_notm}} 인프라 모바일
![외부 링크 아이콘](../icons/launch-glyph.svg)](https://knowledgelayer.softlayer.com/topic/mobile-devices){:new_window}을 사용하여
{{site.data.keyword.BluSoftlayer_notm}} 인프라 디바이스를 iOS 또는 Android 모바일 디바이스를 통해 계속 관리하십시오. {{site.data.keyword.BluSoftlayer_notm}} 인프라 내 기능에는 티켓팅 지원, 기본 디바이스 제어 및 대역폭 모니터링이 포함됩니다. 
